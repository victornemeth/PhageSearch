...
def fetch_uniprot_ids_from_db():
    ...
    try:
        ...
        cursor.execute("SELECT UniProt_Ref, primary_key FROM proteins")
        ...
        for row in rows:
            ...
            if row[0] is not None:
                ...
                uniprot_id = row[0].split(":")[-1]  # Extract the ID part after the colon
                uniprot_primary_key = row[1]
                uniprot_ids.append((uniprot_id, uniprot_primary_key))
...
def insert_structure_into_database(pdb_id, structure, uniprot_primary_key):
    ...
    try:
        ...
        sql = "INSERT INTO structures (pdb_id, structure, proteins_primary_key) VALUES (%s, %s, %s)"
        cursor.execute(sql, (pdb_id, structure, uniprot_primary_key))
        db.commit()
...
def main():
    ...
    for uniprot_id, uniprot_primary_key in common_uniprot_ids:
        ...
        if pdb_id and pdb_line:
            ...
            structure = fetch_structure_from_pdb(pdb_id)
            if structure:
                # Insert structure into the database
                insert_structure_into_database(pdb_id, structure, uniprot_primary_key)
            else:
